 ...
 final notificationManager = GetIt.instance<INotificationManager>();
  ...
  Future<void> loginUtente() async {
    ...
    setState(() {
      errorMessage = valid
          ? ''
          : AppLocalizations.of(currentContext)?.invalidCredential ?? '';
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)?.signIn ?? 'SignIn'),
      ),
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 16),
            CheckboxListTile(
              title: Text(AppLocalizations.of(context)?.memorizePassword ?? 'Remember Me'),
